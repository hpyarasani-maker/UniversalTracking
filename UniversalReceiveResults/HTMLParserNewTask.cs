...
namespace UniversalReceiveResults
{
    public class HTMLParserNewTask
    {
        ...
        {
            string url = "http://previous.azurewebsites.net/api/callbackrapidtrackingmobile/";
                    ...
                    try
                    {
                        ...
                        foreach (JObject jo in cont)
                        {
                            ...
                            if (device == "desktop")
                            {
                                ...
                                addURLs = getTop100YahooHKDesktopPattern(response).Result;               // 38
                                //addURLs = getTop100YahooJapanDesktopPattern(response).Result;            // 48
                                //addURLs = getTop100NaverDesktopPattern(response).Result;                 // 138
                                //addURLs = getTop100HaoSou360DesktopPattern(response).Result;             // 175
                                //addURLs = getTop100SogouDesktopPattern(response).Result;                   // 276
                                //addURLs = getTop100PriceSearcherPattern(response).Result;                // 340    
                            }

                            else
                            {
                                addURLs = getTop100HaoSou360MobilePattern(response).Result;               // 193        
                                //addURLs = getTop100YahooJapanMobilePattern(response).Result;              // 194
                                //addURLs = getTop100YahooHKMobilePattern(response).Result;                 // 256
                                //addURLs = getTop100SogouMobilePattern(response).Result;                   // 277
                                //addURLs = getTop100SmCnMobilePattern(response).Result;                      // 278
                                //addURLs = getTop100NaverMobilePattern(response).Result;                   // 440  
                            }
